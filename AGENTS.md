...
- From this module only (Java 11 or newer): `mvn -P module-quality clean verify` to run SpotBugs for Chronicle-Threads while reusing the shared rules; `mvn checkstyle:check` uses the parent Checkstyle configuration, including tests.
