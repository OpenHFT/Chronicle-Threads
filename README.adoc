...
- final Pauser pauser: The pausing strategy of this event loop. See link:README.adoc#_pauser[Pauser].
...
| `busy` | Does not pause the event loop; performs busy spinning | Minimises jitter | Uses more CPU, no monitoring support | &#9746; | &#9745;
...
| `yielding` | Very briefly busy spins then yields | Low jitter, can be shared | Uses more CPU | &#9745; | &#9746;
| `balanced` | Performs busy spinning for short periods and then backs off when idle for longer periods | Good balance of busy waiting and back off | Uses less CPU, but more jitter | &#9745; | &#9746;
...
`TimingPauser` interface extends the `Pauser` interface and pauses the current event loop similarly, but it keeps track of accumulated pause time and throws a `TimeoutException` if the specified timeout is exceeded.
