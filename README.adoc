...
- Impose an interval of 100µs for every event loop.
...
The canonical way to use `Pauser`:
...
For the best performance, the default `busy` Pauser mode minimises jitter. However, it does maximise CPU usage. If there are too many threads in `busy` mode, a machine may slow down.
