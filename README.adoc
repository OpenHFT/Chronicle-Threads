...
The `MonitorEventLoop` thread monitors application threads to make sure event loop latency remains within acceptable bounds. The thread monitors latency by measuring the time the `action` method in the `ThreadMonitorEventHandler` takes to run. Whenever a thread runs beyond an acceptable latency limit, the system prints a stack trace.
...
- Consider adding `Jvm.safepoint` calls with the Hotspot JVM.
