...
`LongPauser` implements `Pauser` and `TimingPauser` and has the following signature:

[source, java]
----
public LongPauser(int minBusy, int minCount, long minTime, long maxTime, @NotNull TimeUnit timeUnit)
----
`LongPauser` first busy-loops and if it is loops at least `minBusy` times, then it yields for `minCount` times before it sleeps,
then it initially sleeps for `minTime` and if there is still nothing to do, it sleeps for longer periods of `maxTime`.
...
=== Setting Up Different Pauser Modes

Noteworthy in setting up different modes are the parameters used. The `PauserModes` from above use the following:

* `minBusy` - the min number of times it will go around doing nothing, after this has been reached it will yield
* `minCount` - the number of times it will yield, before it starts to sleep
* `minTime` - the amount of time to sleep (initially)
* `maxTime` - the amount of time subsequently to sleep
* `timeUnit` - the unit of the minTime and maxTime

As an example, this is an illustration of the use of the parameters that will enable sleepy mode on a pauser:
[source, java]
----
...
----
In other words, this Pauser would:

1. Spin for 0 times (`minBusy`)
2. Between 0-100 it will then yield (`minCount`)
3. Above 100, it would pause for 100 (`minTime`)
4. It would subsequently pause for 20_000 (`maxTime`)
5. The minTime and maxTime are in MICROSECONDS (`timeUnit`)
