...
|THR-NF-P-027 (Latency) |<= 10 µs at 99.99 percentile for single-hop handler runs |Profiling harness schedules 10 million iterations with a busy pauser and isolated core.
|THR-NF-P-028 (Jitter) |<= 2 µs peak-to-peak jitter under steady load |Continuous histogram per handler, sampled via monitor loop over 15 minute windows.
