...
Operate :: Once registered, treat handler state as confined to the loop's thread. All mutations should occur inside `action()` or helper methods invoked from that loop.

Dispose :: Use `InvalidEventHandlerException.reusable()` to self-deregister when the handler has completed its lifecycle (THR-FN-008). Ensure downstream resources honour Chronicle Core's `Closeable` and `ReferenceCounted` contracts.
